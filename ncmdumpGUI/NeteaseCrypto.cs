...
namespace ncmdumpGUI
{
    class NeteaseCrypto
    {
        ...
        public NeteaseCrypto(FileInfo fileInfo)
        {
            ...
            {
                byte swap = _keyBox[i];
                byte c = (byte)((swap + last_byte + finalKey[key_offset++]) & 0xff);
        ...
        public void Dump(string destDir)
        {
            ...
            // 将.ncm替换成最终转换的文件后缀名
            string destFileName = string.Format("{0}.{1}", _fileInfo.Name.Substring(0, _fileInfo.Name.Length - 4), _cdata.Format);
